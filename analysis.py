...
class Analysis:
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        fig, axs = plt.subplots(len(data_vars), len(data_vars), sharex=True, sharey=True)
        fig.set_figheight(8)
        fig.set_figwidth(8)
        ...
        for i in range(len(data_vars)): 
            ...
            for j in range(len(data_vars)):
                ...
                axs[i,j].scatter(limData[:,i], limData[:,j], marker = '.') 
        ...
        plt.xticks([],[])   # pass empty sets to xticks 
        plt.yticks([],[])   # pass empty sets to yticks 
